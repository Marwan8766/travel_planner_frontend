...
function saveProfile() {
    ...
    xhr.onload = function() {
        if (this.status === 200) console.log(this.responseText); // log response to console
    };
...
async function getMe() {
    ...
    for(const key in USerData)$(`#${key}`).val(USerData[key]);
    $("#userName").html(USerData.name);
    $("#userEmail").html(USerData.email);
    if (USerData.image) $("#userImage").attr("src", USerData.image);
...
},{}]},["9L2rn","coHEg"], "coHEg", "parcelRequire6b74")
